...
class ObjectMaskTriangulator(Node):
    ...
    def extract_data_from_obj_mask_msg(self, obj_mask_msg):
        ...
        for cam_id in range(self.num_cameras):
            ...
            expected_cam_frame = cam_frame.lstrip("/")
            camera_info_frame = camera_infos[cam_id].header.frame_id.lstrip("/")
            camera_tf_child_frame = camera_tfs[cam_id].child_frame_id.lstrip("/")

            assert camera_info_frame == expected_cam_frame, \
                f"CameraInfo frame_id {camera_infos[cam_id].header.frame_id} does not match expected {cam_frame}"
            
            assert camera_tf_child_frame == expected_cam_frame, \
