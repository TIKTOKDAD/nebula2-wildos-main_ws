...
class WildOS_Nav(TFLookupSubscriber):
    default_config = {
        ...
        # `/nav_graph_viz` 只用于调试拓扑，默认压平成 XY 平面，避免 2.5D 高度影响判断。
        "flatten_nav_graph_viz": True,

        # 压平显示时使用的固定 Z，高于地面一点可减少与 GridMap/Marker 的深度闪烁。
        "nav_graph_viz_z": 0.08,
    ...
    def __init__(self, config: OmegaConf=OmegaConf.create(), do_object_search=False):
        ...
        self.viz = VisualizeGoalAgnosticGeoFrontierScoring(
            ...
            flatten_nav_graph_viz=config.flatten_nav_graph_viz,
            nav_graph_viz_z=config.nav_graph_viz_z,
        )
    ...
    def init_model(self, config, do_object_search):
        ...
        self.model = ExploRFMInference(
            ...
            adaptor_ckpt_path=HOME_DIR / "ckpts" / "siglip2",
